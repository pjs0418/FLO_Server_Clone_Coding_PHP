...
function getPlayingSongs($songIdx, $characterIdx)
{
    $pdo = pdoSqlConnect();
    $query = "select contents             as songContents,
       length               as songLength,
       lyrics               as songLyrics,
       isCharacterLikedSong as songLikedInfo,
       AlbumInfo.title                as albumTitle,
       lyricist,
       composer,
       arranger
from Song
         inner join (select isCharacterLikedSong, songIdx from CharacterSongLiked where characterIdx = ?) LikedInfo
                    on LikedInfo.songIdx = Song.idx
         inner join (select idx, title from Album where isDeleted = 'N') AlbumInfo
                    on AlbumInfo.idx = Song.albumIdx
where Song.idx = ?
  AND isDeleted = 'N';";

    $st = $pdo->prepare($query);
    $st->execute([$characterIdx, $songIdx]);
    //    $st->execute();
...
function addListeningCount($songIdx)
{
    $pdo = pdoSqlConnect();
    $query = "UPDATE Song SET listeningCount = listeningCount + 1 where idx = ?;";
...
function isExistHistory($characterIdx, $songIdx)
{
    $pdo = pdoSqlConnect();
    $query = "select count(1) as exist from History where characterIdx = ? AND songIdx = ?;";
...
function addHistory($characterIdx, $songIdx)
{
    $pdo = pdoSqlConnect();
    $query = "INSERT INTO History (characterIdx, songIdx) VALUES (?, ?);";
...
function addCountHistory($characterIdx, $songIdx)
{
    $pdo = pdoSqlConnect();
    $query = "UPDATE History SET listeningCount = listeningCount + 1 where characterIdx = ? AND songIdx = ?;";
