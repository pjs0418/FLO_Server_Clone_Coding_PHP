...
try {
    ...
    switch ($handler) {
        ...
        case "getCharts":
            ...
            $res->result = getCharts();
            $res->isSuccess = TRUE;
            $res->code = 1000;
            $res->message = "둘러보기 화면 차트 정보 조회 성공";
        ...
        case "getMixedCharts":
            ...
            $characterMostListenedSongClassificationIdx = getCharacterMostListenedSongClassificationIdx($currentCharacterIdx);

            $recommendedSongsIdx = getRecommendedSongsIdx($characterMostListenedSongClassificationIdx);

            resetMixedChart($currentCharacterIdx);
            for($i = 0;$i < count($recommendedSongsIdx);$i++)
            {
                if(isExistSongInChart($recommendedSongsIdx[$i]['songIdx']))
                {
                    addSongInMixedChart($recommendedSongsIdx[$i]['songIdx'], $currentCharacterIdx);
                }
            }

            $songsInChartIdx = getSongsInChartIdx();

            for($i = 0;$i < count($songsInChartIdx);$i++)
            {
                if(!isExistSongInMixedChart($songsInChartIdx[$i]['songIdx'], $currentCharacterIdx))
                {
                    addSongInMixedChart($songsInChartIdx[$i]['songIdx'], $currentCharacterIdx);
                }
            }
            $res->result = getSongsInMixedChart($currentCharacterIdx);
            $res->isSuccess = TRUE;
            $res->code = 1000;
            $res->message = "추천 MIX 적용 차트 정보 조회 성공";
            echo json_encode($res, JSON_NUMERIC_CHECK);
            break;
