...
try {
    ...
    switch ($handler) {
        ...
        case "getHomes":
            ...
            if($jwt == null)
            {
                $res->result->mainResult = getHomesBeforeChoice();
                $res->result->todayReleasedMusic = getTodayReleasedMusic();
                $res->result->recommendedLists = getRecommendedLists();
                $res->isSuccess = TRUE;
                $res->code = 1000;
                $res->message = "홈화면 정보 조회 성공(비로그인/취향 선택 X)";
            ...
            if(!isCharacterTaste($currentCharacterIdx))
            {
                $res->result->mainResult = getHomesBeforeChoice();
                $res->result->todayReleasedMusic = getTodayReleasedMusic();
                $res->result->recommendedLists = getRecommendedLists();
            ...
            $res->result->mainResult = getHomesAfterChoice($currentCharacterIdx);
            $res->result->todayReleasedMusic = getTodayReleasedMusic();
            $res->result->recommendedLists = getRecommendedLists();
            $res->isSuccess = TRUE;
            $res->code = 1001;
            $res->message = "홈화면 정보 조회 성공(취향 선택 O)";
            echo json_encode($res, JSON_NUMERIC_CHECK);
            break;
//        case "modifyTastes":
//            http_response_code(200);
